...
def table_args():
    engine_name = urlparse(cfg.CONF.database_alarm_connection).scheme
...
class Alarm(Base):
    ...
    alarm_metadatas = relationship("AlarmMetadata", backref="alarm")


class AlarmMetadata(Base):
    __tablename__ = 'alarm_metadata'
    alarm_id = Column(String(255), ForeignKey('alarm.id'), primary_key=True)
