...
sql_opts = [
    cfg.IntOpt('alarm_sql_connection_debug',
               default=0,
               help='Verbosity of SQL debugging information. 0=None, '
                    '100=Everything'),
    cfg.BoolOpt('alarm_sql_connection_trace',
                default=False,
                help='Add python stack traces to SQL as comment strings'),
    cfg.BoolOpt('alarm_sqlite_synchronous',
                default=True,
                help='If passed, use synchronous mode for sqlite'),
    cfg.IntOpt('alarm_sql_idle_timeout',
               default=3600,
               help='timeout before idle sql connections are reaped'),
    cfg.IntOpt('alarm_sql_max_retries',
               default=10,
               help='maximum db connection retries during startup. '
                    '(setting -1 implies an infinite retry count)'),
    cfg.IntOpt('alarm_sql_retry_interval',
               default=10,
...
def get_engine():
    ...
    if _ENGINE is None:
        connection_dict = sqlalchemy.engine.url.make_url(
            cfg.CONF.alarm_database_connection)

        engine_args = {
            "pool_recycle": cfg.CONF.alarm_sql_idle_timeout,
            "echo": False,
            'convert_unicode': True,
        }

        # Map our SQL debug level to SQLAlchemy's options
        if cfg.CONF.alarm_sql_connection_debug >= 100:
            engine_args['echo'] = 'debug'
        elif cfg.CONF.alarm_sql_connection_debug >= 50:
            engine_args['echo'] = True

        if "sqlite" in connection_dict.drivername:
            engine_args["poolclass"] = NullPool

            if cfg.CONF.alarm_database_connection == "sqlite://":
                engine_args["poolclass"] = StaticPool
                engine_args["connect_args"] = {'check_same_thread': False}

        _ENGINE = sqlalchemy.create_engine(cfg.CONF.alarm_database_connection,
                                           **engine_args)

        if 'mysql' in connection_dict.drivername:
            sqlalchemy.event.listen(_ENGINE, 'checkout', ping_listener)
        elif "sqlite" in connection_dict.drivername:
            if not cfg.CONF.alarm_sqlite_synchronous:
                sqlalchemy.event.listen(_ENGINE, 'connect',
                                        synchronous_switch_listener)
            sqlalchemy.event.listen(_ENGINE, 'connect', add_regexp_listener)

        if (cfg.CONF.alarm_sql_connection_trace and
        ...
        try:
            ...
            remaining = cfg.CONF.alarm_sql_max_retries
            if remaining == -1:
                remaining = 'infinite'
            while True:
                ...
                if remaining != 'infinite':
                    remaining -= 1
                time.sleep(cfg.CONF.alarm_sql_retry_interval)
                try:
                    _ENGINE.connect()
                    break
                except OperationalError, e:
